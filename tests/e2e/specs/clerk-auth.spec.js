...
test.describe('Clerk Authentication', () => {
  ...
  test.describe('Protected Routes', () => {
    test('should redirect to home when accessing dashboard without authentication', async ({ page }) => {
      // Try to access protected dashboard page
      await page.goto('/dashboard');
      
      // Should be redirected to home page or sign-in
      await page.waitForURL(url => url.pathname === '/' || url.pathname.includes('sign-in'), { timeout: 5000 });
  ...
  test.describe('API Route Protection', () => {
    ...
    test('should return 401 for POST requests to protected API route when not authenticated', async ({ page }) => {
