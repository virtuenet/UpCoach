...
exports.personalityEngine = exports.PersonalityEngine = void 0;
...
Balance directness with respect and understanding of individual circumstances.`,
    ...
    addPersonality(profile) {
        this.personalities.set(profile.id, profile);
    }
    getSystemPrompt(personalityId = 'default') {
    ...
    selectOptimalPersonality(userContext) {
        ...
        if (userContext.preferredMethods?.includes('data') ||
    ...
    enhanceResponse(response, personalityId, responseType) {
    ...
    adjustTone(message, personalityId) {
        const personality = this.getPersonality(personalityId);
    ...
    async trackPersonalityEffectiveness(personalityId, userId, metrics) {
        // In production, store this data for analysis
        logger_1.logger.info('Personality effectiveness tracked:', {
            personalityId,
            userId,
            metrics,
        });
    ...
    async getPersonalityRecommendation(userId, userHistory) {
        ...
        if (avgSessionDuration < 5)
            return 'direct';
        if (preferredTopics.includes('emotional_support'))
            return 'empathetic';
        if (preferredTopics.includes('data') || preferredTopics.includes('metrics'))
            return 'analytical';
        if (engagementStyle === 'high_energy')
            return 'motivational';
        return 'default';
    }
}
...
//# sourceMappingURL=PersonalityEngine.js.map