...
class RecommendationEngine {
    ...
    async generateRecommendations(userId, types = ['habit', 'goal', 'task', 'wellness'], limit = 5) {
        try {
            // Get user profile
            ...
            const personalizedRecommendations = await this.personalizeRecommendations(sortedRecommendations, context);
    ...
    async buildRecommendationContext(userId, profile) {
        const now = new Date();
        ...
        if (hour < 6)
            ...
            timeOfDay = 'night';
        // Get recent mood
        const recentMood = await Mood_1.Mood.findOne({
            where: { userId },
    ...
    async generateHabitRecommendations(context) {
    ...
    async generateGoalRecommendations(context) {
        const recommendations = [];
        const profile = context.recentActivity.profile;
        // Get current goals
        const currentGoals = await Goal_1.Goal.findAll({
            where: {
    ...
    async generateWellnessRecommendations(context) {
        const recommendations = [];
    ...
    hasRecentReflectionActivity(tasks) {
        return tasks.some(task => task.title.toLowerCase().includes('reflect') ||
            task.title.toLowerCase().includes('journal') ||
            task.title.toLowerCase().includes('review'));
    }
    hasFragmentedTasks(tasks) {
        if (tasks.length < 5)
            return false;
    ...
    formatGrowthArea(area) {
        const formatted = area.replace(/_/g, ' ').replace(/-/g, ' ');
        return formatted.charAt(0).toUpperCase() + formatted.slice(1);
    }
    async getOptimalTiming(userId, activityType) {
        const profile = await UserProfilingService_1.userProfilingService.createOrUpdateProfile(userId);
        ...
        const timingMap = {
            ...
            exercise: {
                bestTime: profile.behaviorPatterns?.avgSessionDuration &&
    ...
    async generateAdaptiveSchedule(userId, _date) {
