...
exports.insightGenerator = exports.InsightGenerator = void 0;
...
class InsightGenerator {
    ...
    async generateInsightReport(userId, period = { days: 30 }) {
        try {
            // Determine period
            ...
            const recommendations = await this.generateReportRecommendations(rankedInsights, trends, userData);
            const report = {
                userId,
    ...
    async gatherUserData(userId, startDate, endDate) {
    ...
    async calculateMetrics(_userId, goals, tasks, moods) {
    ...
    async generateAnomalyInsights(data) {
        const insights = [];
    ...
    async generatePredictionInsights(data) {
        const insights = [];
        // Goal completion predictions
        for (const goal of data.goals.filter((g) => g.status === 'active')) {
            const prediction = await PredictiveAnalytics_1.predictiveAnalytics.predictGoalCompletion(goal.id);
        ...
        // Success prediction
        const successPrediction = await PredictiveAnalytics_1.predictiveAnalytics.predictUserSuccess(data.userId);
    ...
    async generateRecommendationInsights(data) {
        ...
        const recommendations = await RecommendationEngine_1.recommendationEngine.generateRecommendations(data.userId, ['habit', 'goal', 'wellness'], 3);
    ...
    async generateAchievementInsights(data) {
        const insights = [];
        // Streak achievements
    ...
    async generateRiskInsights(data) {
        const insights = [];
        // Churn risk
        const churnRisk = await PredictiveAnalytics_1.predictiveAnalytics.predictChurnRisk(data.userId);
    ...
    async generateOpportunityInsights(data) {
        const insights = [];
    ...
    analyzeTaskPatterns(tasks) {
        ...
        if (peakHours.length > 0 && peakHours[0].completionRate > 0.7) {
            const timeframe = peakHours[0].hour < 12 ? 'morning' : peakHours[0].hour < 17 ? 'afternoon' : 'evening';
    ...
    buildProductivityHeatmap(tasks) {
        const heatmap = {};
    ...
    detectMoodAnomaly(moods) {
        ...
        const negativeCount = recentMoods.filter((m) => m.mood === 'stressed' || m.mood === 'sad').length;
        if (negativeCount >= 5) {
            return {
    ...
    generateProgressProjection(goal, _prediction) {
    ...
    mapRecommendationCategory(category) {
        const mapping = {
            wellness: 'wellbeing',
    ...
    async analyzeTrends(userData) {
        const trends = [];
        // Task completion trend
        const taskTrend = this.calculateTrend(userData.tasks, 'createdAt', t => t.status === 'completed');
        trends.push({
    ...
    calculateTrend(items, _dateField, filterFn) {
    ...
    calculateMoodTrend(moods) {
        ...
        const secondAvg = secondHalf.reduce((sum, m) => sum + (moodValues[m.mood] || 3), 0) /
            secondHalf.length;
        const change = ((secondAvg - firstAvg) / firstAvg) * 100;
        return {
            direction: change > 5 ? 'up' : change < -5 ? 'down' : 'stable',
            percentage: Math.round(change),
            interpretation: change > 5
    ...
    calculateEngagementTrend(userData) {
        const totalActivities = userData.tasks.length + userData.goals.length + userData.moods.length;
        const daysInPeriod = Math.max(1, (new Date(userData.period.endDate).getTime() -
        ...
        return {
            ...
            interpretation: dailyEngagement > 3
    ...
    calculateMoodVariability(moods) {
        if (moods.length < 2)
            return 0;
        const moodValues = {
            happy: 5,
    ...
    async generateReportRecommendations(insights, trends, userData) {
    ...
    cacheInsights(userId, insights) {
        ...
        insights.forEach(insight => {
            if (!insight.expiresAt) {
                ...
                insight.expiresAt = new Date(Date.now() + (expirationDays[insight.type] || 7) * 24 * 60 * 60 * 1000);
            }
        });
    }
...
exports.insightGenerator = new InsightGenerator();
