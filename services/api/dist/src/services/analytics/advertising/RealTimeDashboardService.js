...
class RealTimeDashboardService extends events_1.EventEmitter {
    ...
    metricsCache;
    alertThresholds;
    constructor(logger, redis, db, platformService, predictiveService) {
        super();
        this.logger = logger;
        this.redis = redis;
        this.db = db;
        ...
        this.performanceMonitor = new PerformanceMonitor_1.PerformanceMonitor(logger, redis);
    ...
    async refreshDashboardMetrics() {
        ...
        try {
            ...
            await this.redis.setex('dashboard:metrics:latest', 300, JSON.stringify(dashboardMetrics));
    ...
    async fetchTopCampaigns(limit = 10) {
        const query = `
      SELECT
        c.campaign_id as id,
    ...
    async handleMetricsRequest(clientId, data) {
        const client = this.connectedClients.get(clientId);
        if (!client)
            return;
        try {
    ...
    async fetchCustomMetrics(filters, options) {
        let query = `
    ...
    async fetchMetricWidget(config) {
    ...
    async fetchHeatmapWidget(config) {
    ...
    async fetchFunnelWidget(config) {
    ...
    async fetchTimeSeriesData(metrics, dateRange) {
    ...
    async fetchBarChartData(metrics, groupBy, dateRange) {
        const metricSelects = metrics.map(m => `SUM(${m}) as ${m}`).join(', ');
        const query = `
      SELECT
        ${groupBy} as category,
    ...
    async fetchPieChartData(metric, groupBy, dateRange) {
        const query = `
      SELECT
        ${groupBy} as label,
        SUM(${metric}) as value
