...
class Avatar extends sequelize_1.Model {
    ...
    updatedBy;
    // Instance methods
    calculateCompatibilityScore(traits) {
        let score = 0;
        let totalCriteria = 0;
        ...
        for (const [trait, criteria] of Object.entries(this.personality.recommendedFor.traits)) {
            ...
            if (criteria.min !== undefined && criteria.max !== undefined) {
                ...
                if (userValue >= criteria.min && userValue <= criteria.max) {
                    ...
                    const distance = Math.min(Math.abs(userValue - criteria.min), Math.abs(userValue - criteria.max));
    ...
    getExpressionForMood(mood) {
        if (mood >= 8)
            return this.visuals.expressions.excited;
        if (mood >= 6)
            return this.visuals.expressions.happy;
        if (mood >= 4)
            return this.visuals.expressions.encouraging;
        if (mood >= 2)
            return this.visuals.expressions.concerned;
        return this.visuals.expressions.thinking;
    }
    isCompatibleWithMBTI(mbtiType) {
        return this.personality.recommendedFor.mbtiTypes?.includes(mbtiType) || false;
    }
    isCompatibleWithDISC(discProfile) {
        return this.personality.recommendedFor.discProfiles?.includes(discProfile) || false;
    }
    // Static methods
    static async getActiveAvatars() {
        return this.findAll({
            where: { isActive: true },
            order: [
                ['sortOrder', 'ASC'],
    ...
    static async getRecommendedAvatars(traits, limit = 3) {
    ...
    static async findByPersonalityType(traits, mbtiType, discProfile) {
        const avatars = await this.getActiveAvatars();
    ...
    static async getPopularAvatars(limit = 5) {
    ...
    static async seedDefaultAvatars() {
...
Avatar.init({
    ...
    category: {
        type: sequelize_1.DataTypes.ENUM('mentor', 'friend', 'coach', 'guide', 'specialist'),
        allowNull: false,
    },
    personality: {
