...
>({
  ...
  sizeCalculation: (value: { result: unknown; timestamp: number }) => {
  ...
  dispose: (value: { result: unknown; timestamp: number }, key: string, reason: string) => {
...
function generateCacheKey(schema: ZodSchema, data: unknown, schemaName: string): string {
...
export function validateMultiple(
  ...
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const allErrors: Array<{ source: ValidationSource; errors: Array<unknown> }> = [];
