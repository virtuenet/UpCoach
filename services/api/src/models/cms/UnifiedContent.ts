  ...
  implements UnifiedContentAttributes
{
  ...
  public readonly category?: unknown; // ContentCategory
  public readonly parent?: UnifiedContent;
  public readonly children?: UnifiedContent[];
  public readonly tags?: unknown[]; // ContentTag[]
  public readonly media?: unknown[]; // ContentMedia[]
  public readonly comments?: unknown[]; // ContentComment[]
  public readonly interactions?: unknown[]; // ContentInteraction[]

  // Association methods
  public setTags!: (tags: unknown[]) => Promise<void>;
  public getTags!: () => Promise<any[]>;
  public addTag!: (tag: unknown) => Promise<void>;
  public removeTag!: (tag: unknown) => Promise<void>;
  ...
  static async getPublished(type?: ContentType): Promise<UnifiedContent[]> {
    const where: unknown = { status: 'published' };
  ...
  static async getFeatured(limit = 10): Promise<UnifiedContent[]> {
    return UnifiedContent.findAll({
      where: {
        status: 'published',
        featuredImageUrl: { [Op.ne as unknown]: null },
      },
      order: [['publishedAt', 'DESC']],
      limit,
    });
