  ...
  implements ContentAnalyticsAttributes
{
  ...
  static async getUserEngagement(userId: string): Promise<{
    ...
    analytics.forEach((record: unknown) => {
  ...
  static async getContentPerformance(
    ...
    const uniqueUsers = new Set(analytics.filter(a => a.userId).map((a: unknown) => a.userId)).size;
    const views = analytics.filter((a: unknown) => a.event === 'view');
    const completes = analytics.filter((a: unknown) => a.event === 'complete');

    const readTimes = analytics
      .filter((a: unknown) => a.metadata?.readTime)
      .map((a: unknown) => a.metadata.readTime);

    const referrers = analytics
      .filter((a: unknown) => a.metadata?.referrer)
      .map((a: unknown) => a.metadata.referrer);

    const devices = analytics
      .filter((a: unknown) => a.metadata?.deviceType)
      .map((a: unknown) => a.metadata.deviceType);

    const locations = analytics
      .filter((a: unknown) => a.metadata?.location?.country)
      .map((a: unknown) => a.metadata.location.country);
  ...
  private static calculateEngagementScore(analytics: unknown[]): number {
    let score = 0;
    const weights = {
      view: 1,
      read: 3,
