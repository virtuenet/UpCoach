  ...
  implements ExperimentEventAttributes
{
  ...
  static initializeModel: (sequelize: unknown) => typeof ExperimentEvent;
  ...
  static async getEventMetrics(
    ...
    const [countResult, uniqueUsersResult, valueResult] = await Promise.all([
      ...
      this.findAll({
        where: whereClause,
        attributes: [
          [(this as unknown).sequelize.fn('AVG', (this as unknown).sequelize.col('eventValue')), 'averageValue'],
          [(this as unknown).sequelize.fn('SUM', (this as unknown).sequelize.col('eventValue')), 'totalValue'],
        ],
        raw: true,
      }),
    ]);
...
ExperimentEvent.initializeModel = function(sequelizeInstance: unknown) {
