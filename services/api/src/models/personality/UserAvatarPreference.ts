  ...
  implements UserAvatarPreferenceAttributes
{
  ...
  > {
    ...
    return preferences.map((p: unknown) => ({
      avatarId: p.avatarId,
  ...
  static async getRecommendationsForUser(userId: string): Promise<{
    ...
    if (userProfile) {
      ...
      const compatibleAvatars = await (Avatar as unknown).findByPersonalityType(
        (userProfile as unknown).traits
      );

      compatibleAvatars.slice(0, 2).forEach((avatar: unknown) => {
