...
export function createCircuitBreaker<T extends (...args: unknown[]) => Promise<unknown>>(
  ...
  const breaker = new CircuitBreaker<any[], any>(fn as unknown, breakerOptions);
...
export class CircuitBreakerWrapper extends EventEmitter {
  ...
  constructor(options: CircuitBreakerOptions = {}) {
    ...
    const dummyFn = async (..._args: unknown[]) => {
    ...
    this.breaker.on('success', (result: unknown) => {
  ...
  getState(): 'CLOSED' | 'OPEN' | 'HALF_OPEN' {
    if ((this.breaker as unknown).opened) return 'OPEN';
    if ((this.breaker as unknown).halfOpen) return 'HALF_OPEN';
    return 'CLOSED';
  }
