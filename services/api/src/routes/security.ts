...
router.get('/events', checkResourceAction(['read']), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error fetching security events', { error, userId: (req as any).user?.id });
...
router.get('/metrics', checkResourceAction(['read']), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error fetching security metrics', { error, userId: (req as any).user?.id });
...
router.post('/test-event', checkResourceAction(['create']), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error creating test security event', { error, userId: (req as any).user?.id });
...
router.get('/compliance/export', checkResourceAction(['read']), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error exporting compliance data', { error, userId: (req as any).user?.id });
...
router.post('/cleanup', checkResourceAction(['create']), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error cleaning up security events', { error, userId: (req as any).user?.id });
