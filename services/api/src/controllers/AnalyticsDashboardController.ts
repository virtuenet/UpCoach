...
export class AnalyticsDashboardController {
  ...
  async getBehaviorPatterns(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const patterns = await UserBehaviorAnalyticsService.analyzeBehaviorPatterns(userId, {
        ...
        patternTypes: patternTypes ? (patternTypes as string).split(',') as any : undefined,
  ...
  private async getUserComparisons(userId: string): Promise<any> {
    return {
      percentile: 75,
      peerAverage: 65,
