...
export const getArticle = async (req: Request, _res: Response) => {
  try {
    ...
    const where: unknown = isNaN(Number(id)) ? { slug: id } : { id };
...
function analyzeSEO(title: string, content: string): unknown {
  const issues = [];
  const suggestions = [];
...
async function generatePreview(content: unknown, format: string): Promise<string> {
