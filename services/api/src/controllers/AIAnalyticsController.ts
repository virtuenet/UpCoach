...
export class AIAnalyticsController {
  async getAIMetrics(req: Request, _res: Response): Promise<void> {
    try {
      ...
      const results = totalInteractionsResult as unknown;
      ...
      const satisfactionRate = parseFloat((satisfactionResult as unknown).satisfaction_rate) || 85; // Default to 85%
      ...
      const modelUsage = modelUsageResult as unknown;
      ...
      const previousTokens = parseInt((previousCostResult as unknown).total_tokens) || 1;
  ...
  async getAIInteractions(req: Request, _res: Response): Promise<void> {
    try {
      ...
      _res.json(
        interactions.map((interaction: unknown) => ({
          id: interaction.id,
    ...
    try {
      ...
      const filledData = dateRange.map(date => {
        ...
        const existing = usageData.find(
          (d: unknown) => format(new Date(d.date), 'yyyy-MM-dd') === dateStr
        );

        return {
          date: dateStr,
          conversations: (existing as unknown)?.conversations || 0,
          recommendations: (existing as unknown)?.recommendations || 0,
          voiceAnalysis: (existing as unknown)?.voice_analysis || 0,
        };
      });
