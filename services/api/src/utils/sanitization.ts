 ...
 */
export function sanitizeString(input: string, logSuspicious = true): string {
 ...
 */
export function sanitizeObject<T>(obj: T, strict = true): T {
  ...
  if (typeof obj === 'string') {
    return (strict ? sanitizeString(obj, false) : sanitizeRichText(obj)) as T;
  }

  if (Array.isArray(obj)) {
    return obj.map(item => sanitizeObject(item, strict)) as T;
  }
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
        logger.warn('Potential prototype pollution attempt detected', {
          key,
          ip: (global as unknown).currentRequestIp,
        });
        continue;
      }
...
export function createSanitizationReport(
  ...
  function compareObjects(orig: unknown, san: unknown, path = ''): void {
