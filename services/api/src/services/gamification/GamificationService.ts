...
export class GamificationService {
  ...
  async initializeUser(userId: number, transaction?: Transaction): Promise<void> {
    try {
      ...
      // Initialize common streaks - Optimized bulk insert (3 queries → 1 query)
      await sequelize.query(
        `INSERT INTO user_streaks (user_id, streak_type)
         VALUES
           (:userId, 'daily_login'),
           (:userId, 'weekly_goal'),
           (:userId, 'mood_tracking')
  ...
  private async checkLevelUp(userId: number, transaction?: Transaction): Promise<void> {
    ...
    if ((result[0] as unknown as unknown[])?.length > 0) {
      const newLevel = (result[0] as unknown as unknown[])[0].current_level;
  ...
  private async onLevelUp(userId: number, newLevel: number): Promise<void> {
    // Get level details
    const levelInfo = await sequelize.query<unknown>(
    ...
    if (levelInfo[0]) {
      const level = levelInfo[0] as unknown;
  ...
  async trackProgress(
    ...
    try {
      ...
      const achievements = await sequelize.query<unknown>(
      ...
      for (const achievement of achievements) {
        await this.updateAchievementProgress(userId, achievement as unknown, value, metadata);
      }
    } catch (error) {
  ...
  private async unlockAchievement(userId: number, achievementId: number): Promise<void> {
    try {
      ...
      const achievement = await sequelize.query<unknown>(
      ...
      if (achievement[0]) {
        const ach = achievement[0] as unknown;
  ...
  async updateStreak(
    ...
    try {
      ...
      // Get current streak
      const streakData = await sequelize.query<unknown>(
      ...
      if (streakData[0]) {
        const streak = streakData[0] as unknown;
  ...
  private async checkStreakAchievements(
    ...
    for (const achievement of achievements) {
      await this.unlockAchievement(userId, (achievement as unknown).id);
    }
  }
  ...
  async getUserStats(userId: number): Promise<UserStats> {
    try {
      const stats = await sequelize.query<unknown>(
        `SELECT 
           ul.*,
      ...
      return {
        ...
        currentStreak: (streaks[0] as unknown)?.max_streak || 0,
        rank: userStats.rank,
      };
    } catch (error) {
  ...
  async getUserAchievements(userId: number, category?: string): Promise<AchievementProgress[]> {
    try {
      ...
      return achievements.map((ach: unknown) => ({
  ...
  async updateChallengeProgress(
    ...
    try {
      ...
      const current = await sequelize.query<unknown>(
      ...
      if (current[0]) {
        const progressData = (current[0] as unknown).progress || {};
        const requirements = (current[0] as unknown).requirements;
  ...
  private async onChallengeComplete(userId: number, challengeId: number): Promise<void> {
    const challenge = await sequelize.query<unknown>(
    ...
    if (challenge[0]) {
      ...
      await this.awardPoints(
        userId,
        (challenge[0] as unknown).reward_points,
        `Challenge completed: ${(challenge[0] as unknown).name}`
      );
  ...
  async getLeaderboard(
    ...
    try {
      let query = '';
      const replacements: unknown = { limit };
      ...
      const results = await sequelize.query<unknown>(query, {
        replacements,
        type: QueryTypes.SELECT,
      });
    ...
    try {
      ...
      try {
        ...
        const [rewardItem, userLevel] = await Promise.all([
          ...
          sequelize.query<unknown>(`SELECT * FROM user_levels WHERE user_id = :userId`, {
            replacements: { userId },
            type: QueryTypes.SELECT,
            transaction: t,
          }),
        ...
        const item = rewardItem[0] as unknown;
        const user = userLevel[0] as unknown;
        ...
        const purchaseCount = await sequelize.query<unknown>(
        ...
        if ((purchaseCount[0] as unknown).count >= item.purchase_limit_per_user) {
  ...
  private async getUserEmail(userId: number): Promise<string> {
    const user = await sequelize.query<unknown>(`SELECT email FROM users WHERE id = :userId`, {
      replacements: { userId },
      type: QueryTypes.SELECT,
    });
    return (user[0] as unknown)?.email || '';
  }

  private async notifyAchievementUnlock(userId: number, achievement: unknown): Promise<void> {
