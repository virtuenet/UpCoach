...
export class ABTestingService {
   ...
   * Check if user meets segmentation criteria
   */
  private meetsSegmentationCriteria(user: unknown, segmentation?: unknown): boolean {
    if (!segmentation) return true;

    // Check include rules
    if (segmentation.includeRules && segmentation.includeRules.length > 0) {
      const meetsIncludeRules = segmentation.includeRules.every((rule: unknown) =>
    ...
    if (segmentation.excludeRules && segmentation.excludeRules.length > 0) {
      const meetsExcludeRules = segmentation.excludeRules.some((rule: unknown) =>
        this.evaluateSegmentRule(user, rule)
      );
   ...
   */
  private evaluateSegmentRule(user: unknown, rule: unknown): boolean {
