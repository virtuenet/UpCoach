...
export class MarketingAutomationService {
  ...
  private async matchesAudience(user: User, audience: AudienceFilter): Promise<boolean> {
    ...
    if (audience.properties) {
      for (const [key, value] of Object.entries(audience.properties)) {
        if ((user as unknown)[key] !== value) return false;
      }
    }
  ...
  private async scheduleDelivery(delivery: unknown): Promise<void> {
  ...
  private async deliverCampaign(delivery: unknown): Promise<void> {
...
class SegmentEngine {
  async getUserSegments(userId: string): Promise<string[]> {
    ...
    if (user.subscriptions?.some((s: unknown) => s.status === 'active')) {
    ...
    const recentActivity = user.activities?.filter(
      (a: unknown) => a.createdAt > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    ) || [];
    ...
    // Goal-based segments
    const completedGoals = user.goals?.filter((g: unknown) => g.status === 'completed');
...
class ContentPersonalizer {
  async personalize(content: unknown, user: User): Promise<unknown> {
