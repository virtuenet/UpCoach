...
export class VoiceJournalLocalStorageService extends VoiceJournalService {
   ...
   */
  async storeOfflineEntry(userId: string, entryData: any, audioBuffer?: Buffer): Promise<string> {
    try {
  ...
  private mergeEntries(serverEntry: any, localEntry: any): any {
  ...
  private async storeEntryLocally(entry: any): Promise<void> {
