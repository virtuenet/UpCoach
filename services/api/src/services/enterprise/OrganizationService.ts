...
export class OrganizationService {
  ...
  async updateOrganization(
    ...
    if (data.name && data.name !== organization.name) {
      ...
      const slugExists = await Organization.findOne({
        where: { slug: newSlug, id: { [Op.ne as unknown]: organizationId } },
      });
  ...
  async inviteMember(
    ...
    try {
      ...
      const invitationId = (result[0] as unknown).id;
  ...
  async acceptInvitation(token: string, userId: number): Promise<void> {
    ...
    try {
      ...
      await sequelize.query(
        ...
        {
          replacements: {
            organizationId: (invitation as unknown).organization_id,
            userId,
            role: (invitation as unknown).role,
          },
          transaction,
        }
      );
      ...
      const inv = invitation as unknown;
          ...
          await sequelize.query(
             ...
             VALUES (:teamId, :userId, 'member', :invitedBy)`,
            {
              replacements: {
                teamId,
                userId,
                invitedBy: (invitation as unknown).invited_by,
              },
              transaction,
            }
          );
      ...
      await sequelize.query(
        ...
        {
          replacements: { invitationId: (invitation as unknown).id },
          transaction,
        }
      );
      ...
      logger.info('Organization invitation accepted', {
        invitationId: (invitation as unknown).id,
        organizationId: (invitation as unknown).organization_id,
        userId,
      });
    } catch (error) {
  ...
  async removeMember(organizationId: number, userId: number, removedBy: number): Promise<void> {
    ...
    try {
      ...
      if (removerRole.length === 0 || !['admin', 'owner'].includes((removerRole[0] as unknown).role)) {
  ...
  async getOrganizationStats(organizationId: number): Promise<OrganizationStats> {
    ...
    return {
      totalMembers: parseInt((stats[0] as unknown)?.total_members || '0'),
      activeMembers: parseInt((stats[0] as unknown)?.active_members || '0'),
      totalTeams: parseInt((stats[0] as unknown)?.total_teams || '0'),
      activeTeams: parseInt((stats[0] as unknown)?.active_teams || '0'),
      storageUsed,
      apiCallsThisMonth,
    };
  }
  ...
  async getOrganizationMembers(
    ...
    const replacements: unknown = { organizationId, limit, offset };
