...
export class TeamService {
  async createTeam(data: CreateTeamData): Promise<Team> {
    ...
    try {
      ...
      const currentTeamCount = (teamCount[0] as any).count;
  ...
  async addTeamMember(
    ...
    try {
      ...
      const organizationId = (teams[0] as any).organization_id;
  ...
  async updatePolicy(policyId: number, data: Partial<CreatePolicyData> & { updatedBy: number }): Promise<any> {
    const transaction = await sequelize.transaction();
    ...
    try {
      ...
      const existingPolicy = existingPolicies[0] as any;

      // Build update query dynamically based on provided data
      const updateFields: string[] = [];
      const replacements: any = { policyId, updatedBy: data.updatedBy };
      ...
      const updatedPolicy = result[0] as any;
      return {
        ...updatedPolicy,
  ...
  async togglePolicy(policyId: number, data: { enabled?: boolean; updatedBy: number }): Promise<any> {
    const transaction = await sequelize.transaction();
  ...
  async getTeamLimitsInfo(organizationId: number): Promise<{
    ...
    try {
      ...
      const currentCount = (teamCount[0] as any).count;
      ...
      const plan = subscriptions.length > 0 ? (subscriptions[0] as any).plan : 'free';
  ...
  async checkTeamMemberLimits(organizationId: number, teamId: number): Promise<void> {
    try {
      ...
      const currentMemberCount = (memberCount[0] as any).count;
