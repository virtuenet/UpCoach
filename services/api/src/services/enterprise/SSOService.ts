...
export class SSOService {
  ...
  async createSSOConfiguration(
    ...
    try {
      ...
      const configId = (result[0] as unknown).id;
  ...
  async initiateSAMLLogin(configId: number): Promise<string> {
    ...
    const loginUrl = await saml.getAuthorizeUrlAsync('', {} as unknown, {} as unknown);
  ...
  async handleSAMLCallback(
    ...
    try {
      ...
      const config = configs[0] as unknown;
      ...
      await sequelize.query(
          ...
          :idpSessionId, :attributes, :expiresAt)`,
        {
          replacements: {
            ...
            configId,
            sessionId,
            idpSessionId: (profile as unknown).sessionIndex,
  ...
  async initiateOIDCLogin(configId: number): Promise<string> {
    ...
    const redirectUri = (configs[0] as unknown).oidc_redirect_uri;
  ...
  async initiateSSOLogout(sessionId: string): Promise<string | null> {
    ...
    const session = sessions[0] as unknown;
    ...
    if (session.provider === 'saml') {
      ...
      if ((saml as unknown).getLogoutUrlAsync) {
        return await saml.getLogoutUrlAsync(session.idp_session_id, {} as unknown, {} as unknown);
      } else if ((saml as unknown).getLogoutUrl) {
        return await (saml as unknown).getLogoutUrl({ nameID: session.idp_session_id });
      }
      return null;
    }
  ...
  private async getOIDCClient(configId: number): Promise<unknown> {
  ...
  private async initializeProvider(configId: number): Promise<void> {
    ...
    if (configs.length > 0) {
      const provider = (configs[0] as unknown).provider;
      if (provider === 'saml') {
  ...
  private extractSAMLAttributes(profile: unknown, mapping: unknown): SSOUserAttributes {
  ...
  async updateSSOConfiguration(
    ...
    try {
      ...
      const encryptedUpdates: unknown = { ...updates };
