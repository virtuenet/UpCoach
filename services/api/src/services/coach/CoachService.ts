...
export class CoachService {
  ...
  async getCoachDetails(coachId: number): Promise<CoachProfile | null> {
    try {
      ...
      (coach as unknown).recentReviews = recentReviews.reviews;
      (coach as unknown).reviewStats = stats;
  ...
  async getMarketplaceStats(): Promise<unknown> {
    try {
      const now = new Date();
      ...
      const [totalCoaches, verifiedCoaches, totalSessions, upcomingSessions] = await Promise.all([
        ...
        CoachSession.count({
          where: {
            ...
            status: { [Op.in as unknown]: ['pending', 'confirmed'] },
          },
        }),
      ]);
      ...
      return {
        ...
        totalSessions,
        upcomingSessions,
        totalRevenue: Number((revenueStats[0] as unknown)?.totalRevenue || 0),
        monthlyRevenue: Number((revenueStats[0] as unknown)?.monthlyRevenue || 0),
        averageRating: Number(ratingStats[0]?.averageRating || 0),
        totalReviews: Number((ratingStats[0] as unknown)?.totalReviews || 0),
        sessionsOverTime,
        revenueByCoach: revenueByCoach.map(item => ({
          coach: (item as unknown).coach?.displayName || 'Unknown',
          revenue: Number((item as unknown).revenue || 0),
        })),
        sessionTypes: sessionTypes.map(item => ({
          name: (item as unknown).sessionType,
          value: Number((item as unknown).value || 0),
        })),
        topSpecializations,
      };
    } catch (error) {
  ...
  private async calculateCoachEarnings(
    ...
    const result = sessions[0] as unknown;
    return {
