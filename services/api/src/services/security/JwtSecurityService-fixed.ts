...
/**
 * COMPREHENSIVE JWT SECURITY SERVICE - SECURITY HARDENED
 ...
 * - Fixed security vulnerabilities from original implementation
 */
...
export class JwtSecurityService {
  ...
  /**
   * Encrypt secrets before storage - SECURITY FIXED
   */
  private encryptSecrets(data: string): string {
    ...
    // Use scrypt to derive encryption key with strong salt
  ...
  /**
   * Decrypt secrets from storage - SECURITY FIXED
   */
  private decryptSecrets(encryptedData: string): string {
    try {
  ...
  async generateToken(
    ...
    try {
      ...
      const token = sign(payload, this.secrets.current.secret, {
        algorithm: this.secrets.current.algorithm as any,
        expiresIn,
        issuer: 'upcoach-api',
  ...
  ): Promise<{ valid: boolean; payload?: any; violations?: string[] }> {
    ...
    try {
      ...
      try {
        payload = verify(token, this.secrets.current.secret, {
          algorithms: [this.secrets.current.algorithm as any],
          issuer: 'upcoach-api',
          audience: 'upcoach-client'
        });
      } catch (error) {
        ...
        if (this.secrets.previous) {
          try {
            payload = verify(token, this.secrets.previous.secret, {
              algorithms: [this.secrets.previous.algorithm as any],
              issuer: 'upcoach-api',
              audience: 'upcoach-client'
            });
          } catch (previousError) {
   ...
   */
  private async detectAnomalies(payload: any, metadata: any): Promise<void> {
