...
export class DataExportService {
   ...
   */
  private async collectExportData(exportRequest: ExportRequest): Promise<any> {
    const data: any = {};
    ...
    for (const [table, fields] of Object.entries(ANONYMIZATION_RULES)) {
      if (anonymized[table]) {
        if (Array.isArray(anonymized[table])) {
          anonymized[table] = anonymized[table].map((record: any) =>
   ...
   */
  private anonymizeRecord(record: any, rules: any): any {
  ...
  private async getCoachingSessions(userId: string, dateRange?: any): Promise<any> {
  ...
  private async getSessionNotes(userId: string, dateRange?: any): Promise<any> {
    let query = `
  ...
  private async getAnalyticsData(userId: string, dateRange?: any): Promise<any> {
  ...
  private async getEngagementMetrics(userId: string, dateRange?: any): Promise<any> {
  ...
  private async getUserProgress(userId: string, dateRange?: any): Promise<any> {
