...
export class DatabaseService {
  ...
  private injectedPool: Pool | null = null;
  ...
  /**
   * Inject a Pool instance for testing
   * Must be called before initialize()
   */
  public static injectPool(pool: Pool): void {
  ...
  private async connect(): Promise<void> {
    // If a pool was injected (for testing), use it
    if (this.injectedPool) {
      this.pool = this.injectedPool;
      logger.info('✅ Using injected database pool (test mode)');
      return;
    }
