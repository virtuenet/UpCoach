...
Provide deep AI analysis as JSON:`
   ...
   */
  private calculateCustomKPIs(userId: string, memories: CoachMemory[], goals: KpiTracker[]): any[] {
    const customKpis = [];
  ...
  private calculateWeeklyGoalProgress(
    ...
    for (const goal of goals) {
      ...
      // Method 2: Use performance history data if available
        ...
        const weeklyData = goalData.performanceHistory.filter((entry: any) => {
          const entryDate = new Date(entry.date);
          return entryDate >= weekStart && entryDate <= weekEnd;
        });
        
        if (weeklyData.length > 0) {
          const completedDays = weeklyData.filter((entry: any) => entry.value > 0).length;
  ...
  private async generateWeeklyInsights(
    ...
    if (analytics) {
      const analyticsData = analytics.coachingMetrics as any;
   ...
   */
  private async calculateUserPercentile(userId: string, coachingMetrics: any): Promise<number> {
    try {
   ...
   */
  private calculateConsistentDays(performanceHistory: any[]): number {
