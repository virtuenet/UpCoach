...
export class CoachIntelligenceMLServiceComplete extends EventEmitter {
  ...
  private extractTemporalPatterns(data: any): BehavioralPattern[] {
    ...
    if (data.habits && data.habits.length > 0) {
      ...
      const habitsWithHighCompletion = data.habits.filter((h: any) => h.completionRate > 0.8);
      ...
      // Check for streaks
      const longStreaks = data.habits.filter((h: any) => h.streak > 7);
  ...
  private extractActivityPatterns(data: any): BehavioralPattern[] {
    ...
    if (data.goals && data.goals.length > 0) {
      ...
      const activeGoals = data.goals.filter((g: any) => g.status === 'active' || g.status === 'in_progress');
      const completedGoals = data.goals.filter((g: any) => g.status === 'completed');
      const highProgressGoals = data.goals.filter((g: any) => g.progress > 75);
  ...
  private async extractEmotionalPatterns(userId: string, data: any): Promise<BehavioralPattern[]> {
    ...
    if (data.memories && data.memories.length > 0) {
      ...
      const positiveMoods = data.memories.filter((m: any) => m.sentiment === 'positive');
      const negativeMoods = data.memories.filter((m: any) => m.sentiment === 'negative');
      const neutralMoods = data.memories.filter((m: any) => m.sentiment === 'neutral');
      ...
      if (positiveMoods.length > 0 && negativeMoods.length > 0) {
        ...
        const sentimentChanges = recent.filter((m: any, i: number) => {
          if (i === 0) return false;
