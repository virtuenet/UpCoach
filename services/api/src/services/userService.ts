...
export class UserService {
  ...
  static async updateActiveStatus(id: string, isActive: boolean): Promise<void> {
    try {
      await db.update('users', { is_active: isActive }, { id });
      logger.info('User active status updated:', { userId: id, isActive });
    } catch (error) {
      logger.error('Error updating active status:', error);
      throw new ApiError(500, 'Failed to update user active status');
    }
  }
  ...
  static async getUserStatistics(id: string): Promise<unknown> {
    try {
      const user = await this.findById(id);
      if (!user) {
        throw new ApiError(404, 'User not found');
  ...
  static async verifyGoogleToken(idToken: string): Promise<unknown> {
    try {
