...
export class UnifiedContentService {
  ...
  async createContent(
    ...
    try {
      // Create content
      const content = await UnifiedContent.create(data as unknown, { transaction });
  ...
  async searchContent(
    ...
    if (filter.type) {
      where.type = Array.isArray(filter.type) ? { [Op.in as unknown]: filter.type } : filter.type;
    }

    if (filter.status) {
      where.status = Array.isArray(filter.status)
        ? { [Op.in as unknown]: filter.status }
        : filter.status;
    }
  ...
  async getRelatedContent(contentId: string, limit = 5): Promise<UnifiedContent[]> {
    ...
    const related = await UnifiedContent.findAll({
      where: {
        id: { [Op.ne as unknown]: contentId },
        status: 'published',
        [Op.or as unknown]: [{ categoryId: content.categoryId }, { type: content.type }],
      },
  ...
  private async attachTags(
    ...
    const tags = await UnifiedTag.findAll({
      where: { id: { [Op.in as unknown]: tagIds } },
      transaction,
    });
  ...
  private async attachMedia(
    ...
    await UnifiedMedia.update(
      { contentId },
      {
        where: { id: { [Op.in as unknown]: mediaIds } },
        transaction,
      }
    );
  }
  ...
  private async updateMedia(
    ...
    await UnifiedMedia.update({ contentId: null } as unknown, {
      where: { contentId },
      transaction,
    });
