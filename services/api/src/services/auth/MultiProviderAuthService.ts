...
export class MultiProviderAuthService {
   ...
   */
  private async authenticateWithGoogle(request: UnifiedAuthRequest): Promise<any> {
  ...
  private async authenticateWithApple(request: UnifiedAuthRequest): Promise<any> {
    if (!request.credentials.idToken) {
  ...
  private async authenticateWithFacebook(request: UnifiedAuthRequest): Promise<any> {
  ...
  private async findUserByProviderId(provider: AuthProvider, providerId: string): Promise<any> {
    try {
  ...
  private calculateSecurityScore(user: any, linkedProviders: any[]): number {
    let score = 0;
  ...
  private generateSecurityRecommendations(user: any, linkedProviders: any[]): string[] {
