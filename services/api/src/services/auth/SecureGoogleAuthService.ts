...
export class SecureGoogleAuthService {
   ...
   */
  private async validateAccountLinking(existingUser: any, googleUser: GoogleUserInfo): Promise<void> {
    // Verify email match
  ...
  private async createGoogleUser(googleUser: GoogleUserInfo, deviceInfo?: any): Promise<any> {
    try {
      const result = await db.query(
        `INSERT INTO users (
