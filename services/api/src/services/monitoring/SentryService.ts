...
export class SentryService {
    ...
    app.use(
      Sentry.expressErrorHandler({
        shouldHandleError: error => {
          ...
          if ((error as unknown).status && (error as unknown).status >= 400) {
            return true;
          }
          return true;
        },
   ...
   * Capture message
   */
  captureMessage(message: string, level: Sentry.SeverityLevel = 'info', context?: unknown): string {
    if (!this.initialized) {
