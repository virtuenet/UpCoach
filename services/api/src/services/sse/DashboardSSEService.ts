...
export class DashboardSSEService {
   ...
   */
  async broadcastAnalyticsUpdate(data: any): Promise<void> {
   ...
   */
  async broadcastUserActivity(data: any): Promise<void> {
    await this.broadcastToRole({
      event: 'user_activity',
      data: {
        ...data,
  ...
  private async getDashboardData(client: SSEClient): Promise<any> {
    const data: any = {};
