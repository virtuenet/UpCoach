...
class PerformanceMonitor extends EventEmitter {
  ...
  async measure<T>(name: string, fn: () => Promise<T>, tags?: Record<string, any>): Promise<T> {
  ...
  getAggregatedMetrics(name: string, period: number = 60000): unknown {
  ...
  getRequestStats(period: number = 60000): unknown {
    const cutoff = Date.now() - period;
    ...
    const statusCodes = recentRequests.reduce(
      (acc, r) => {
  ...
  getPrometheusMetrics(): string {
    ...
    if (requests) {
      ...
      lines.push(
        `http_request_duration_seconds_sum ${(requests.avgDuration * requests.total) / 1000}`
      );
