...
export class ReportDeliveryService {
  ...
  private async generateReportData(period: string, reportType: ReportType): Promise<any> {
    const controller = financialDashboardControllerEnhanced;

    const reportData = {
      period,
      generatedAt: new Date(),
      metrics: await (controller as any).getEnhancedDashboardMetrics({ query: { period } }, { json: () => {} }),
      cohortAnalysis: await this.getEnhancedCohortAnalysis(),
      subscriptionAnalytics: await (controller as any).getSubscriptionAnalytics(),
      forecast: await (controller as any).generateRevenueForecasts(),
      optimization: await (controller as any).calculateCostOptimization(),
      kpis: await (controller as any).calculateFinancialKPIs()
    };
  ...
  private generateReportHTML(reportData: any, options: ReportDeliveryOptions): string {
    return `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      ...
      <div class="section">
        ...
        ${reportData.optimization?.recommendations ? reportData.optimization.recommendations.slice(0, 5).map((rec: any) =>
          `<div class="insight">
      ...
      <div class="section">
        ...
        ${reportData.metrics?.alerts ? reportData.metrics.alerts.map((alert: any) =>
  ...
  private flattenReportDataForCSV(reportData: any): any[] {
    const flatData: any[] = [];
    ...
    if (reportData.cohortAnalysis?.cohorts) {
      reportData.cohortAnalysis.cohorts.forEach((cohort: any) => {
        flatData.push({
          Category: 'Cohort',
  ...
  private generateEmailBody(reportData: any, options: ReportDeliveryOptions): string {
    return `
  ...
  private async getMultipleCohortAnalysis(months: number): Promise<any[]> {
    ...
    for (let i = 0; i < months; i++) {
      ...
      try {
        const cohortAnalysis = await (controller as any).analyzeCohort(cohortDate);
  ...
  private generateCohortInsights(cohorts: any[], summary: any): string[] {
