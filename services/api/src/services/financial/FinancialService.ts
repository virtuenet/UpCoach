...
export class FinancialService {
   ...
   */
  async getRevenueByPlan(startDate: Date, endDate: Date): Promise<any> {
  ...
  async generateDailySnapshot(date: Date = new Date()): Promise<FinancialSnapshot> {
    ...
    const costBreakdown = costsByCategory.reduce((acc: any, row: any) => {
   ...
   */
  async getProfitLossStatement(startDate: Date, endDate: Date): Promise<any> {
    // Revenue
    const revenue = await Transaction.sum('amount', {
      where: {
        status: TransactionStatus.COMPLETED,
    ...
    const costsByCategory = costs.reduce((acc: any, row: any) => {
      acc[row.category] = parseFloat(row.total);
      return acc;
    }, {});
