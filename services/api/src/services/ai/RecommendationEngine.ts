...
export class RecommendationEngine {
  private recommendationStrategies: Map<string, (context: unknown) => Promise<Recommendation[]>>;
  ...
  private async generateHabitRecommendations(context: unknown): Promise<Recommendation[]> {
  ...
  private async generateGoalRecommendations(context: unknown): Promise<Recommendation[]> {
    const recommendations: Recommendation[] = [];
  ...
  private async generateTaskRecommendations(context: unknown): Promise<Recommendation[]> {
    const recommendations: Recommendation[] = [];
  ...
  private async generateWellnessRecommendations(context: unknown): Promise<Recommendation[]> {
    const recommendations: Recommendation[] = [];
  ...
  private hasRecentReflectionActivity(tasks: unknown[]): boolean {
    return tasks.some(
      task =>
  ...
  private hasFragmentedTasks(tasks: unknown[]): boolean {
    if (tasks.length < 5) return false;

    // Check if tasks are spread throughout the day
    const taskHours = tasks.map((t: unknown) => new Date(t.createdAt).getHours());
