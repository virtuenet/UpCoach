...
export class ConversationalAI {
  ...
  private async detectIntent(message: string, _context?: unknown): Promise<ConversationIntent> {
  ...
  private async analyzeIntentWithAI(message: string, detectedIntents: unknown[]): Promise<unknown> {
    try {
...
- urgency: low/medium/high`;
  ...
  private async analyzeConversationFlow(history: unknown[], _currentMessage: string): Promise<unknown> {
    ...
    // Calculate metrics
    const userMessages = history.filter((m: unknown) => m.role === 'user');
    const totalLength = userMessages.reduce((sum, m) => sum + m.content.length, 0);
    analysis.avgMessageLength = totalLength / userMessages.length;

    // Question ratio
    const questions = userMessages.filter((m: unknown) => m.content.includes('?'));
  ...
  private buildSystemPrompt(intent: ConversationIntent, personality: string, context: unknown): string {
