...
export class ContextManager {
  ...
  async getUserContext(userId: string): Promise<UserContext> {
    ...
    try {
      ...
      const recentMessages = await ChatMessage.findAll({
        where: {
          chatId: {
            [Op.in as unknown]: sequelize.literal(
  ...
  private calculateStreakDays(tasks: unknown[]): number {
  ...
  private determineProgressStage(
    ...
    if (streakDays > 21 && goalCount > 3) return 'advanced';
    if (streakDays > 7 || goalCount > 1) return 'developing';
    return 'exploring';
  }

  private extractTargetHabit(goals: unknown[], tasks: unknown[]): unknown {
    ...
    const taskTitles = tasks.map((t: unknown) => t.title.toLowerCase());
  ...
  private summarizeDailyRoutine(tasks: unknown[]): string {
  ...
  private calculateMotivationLevel(moods: unknown[], tasks: unknown[]): number {
    const moodScore =
      moods.length > 0 ? moods.reduce((sum, m) => sum + (m.energy || 5), 0) / moods.length : 5;
  ...
  private calculateConsistencyScore(tasks: unknown[]): number {
    if (tasks.length === 0) return 0;

    const daysWithTasks = new Set(tasks.map((t: unknown) => new Date(t.createdAt).toDateString())).size;

    const dayRange = 7; // Look at last 7 days
    return Math.round((daysWithTasks / dayRange) * 100);
  }

  private calculateEngagementLevel(messages: unknown[]): number {
    if (messages.length === 0) return 0;

    const recentMessages = messages.filter(
      (m: unknown) => new Date(m.createdAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
    );
  ...
  private getTodayAccomplishments(tasks: unknown[]): string {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    ...
    return todayTasks.map((t: unknown) => t.title).join(', ');
  }

  private extractChallenges(messages: unknown[]): string {
    ...
    const challengeMessages = messages.filter((m: unknown) =>
  ...
  private identifyCurrentChallenge(goals: unknown[], tasks: unknown[], messages: unknown[]): unknown {
  ...
  private identifyAvailableResources(user: unknown): string {
  ...
  private summarizeRecentConversations(messages: unknown[]): unknown[] {
  ...
  private async extractConversationPatterns(messages: unknown[]): Promise<unknown> {
    const patterns = {
  ...
  private calculateCommunicationFrequency(messages: unknown[]): string {
  ...
  private calculateAverageMessageLength(messages: unknown[]): number {
    if (messages.length === 0) return 0;

    const userMessages = messages.filter((m: unknown) => m.role === 'user');
    const totalLength = userMessages.reduce((sum, m) => sum + m.content.length, 0);

    return Math.round(totalLength / userMessages.length);
  }

  private extractTimePreferences(messages: unknown[]): unknown {
  ...
  async enrichMessages(messages: unknown[], context: unknown): Promise<any[]> {
