...
Consider tone, word choice, and emotional indicators.`;
        ...
        [
          {
            role: 'system',
            content:
              'You are an expert at emotional analysis and sentiment detection. Always respond with valid JSON.',
          },
          {
            role: 'user',
            content: prompt,
  ...
  private async analyzeLinguistics(
    transcript: string
  ): Promise<VoiceAnalysis['linguisticAnalysis']> {
    ...
    const sophisticatedWords = Array.from(uniqueWords).filter(
      w => !commonWords.has(w) && w.length > 6
    );
    ...
    const complexity =
      avgSentenceLength < 10 ? 'simple' : avgSentenceLength > 20 ? 'complex' : 'moderate';
  ...
  private async generateVoiceInsights(
    ...
    if (speechPatterns.fillerWords > 5) {
      insights.push({
        type: 'behavioral',
        insight:
          "You're using several filler words, which might indicate uncertainty or nervousness.",
        confidence: 0.7,
        recommendations: [
    ...
    // Linguistic insights
    if (
      linguisticAnalysis.complexity === 'complex' &&
  ...
  private async storeVoiceSession(
    ...
    const session: VoiceSession = {
      ...
      timestamp: new Date(),
      duration:
        analysis.speechPatterns.speechRate > 0
  ...
  private async updateMoodFromVoice(userId: string, analysis: VoiceAnalysis): Promise<void> {
    try {
      ...
      const energy =
        analysis.speechPatterns.pace === 'fast'
          ? 7
      ...
      await Mood.create({
        ...
        mood,
        moodScore:
          mood === 'great' ? 5 : mood === 'good' ? 4 : mood === 'okay' ? 3 : mood === 'bad' ? 2 : 1,
        energyLevel: energy,
        ...
        [
          {
            role: 'system',
            content:
              'You are a compassionate voice coach who provides personalized guidance based on voice analysis. Be warm, understanding, and actionable.',
          },
          {
            role: 'user',
            content: prompt,
...
Style: ${style}`;
  ...
  private extractKeyThemes(transcript: string): string[] {
    ...
    if (
      transcript.toLowerCase().includes('health') ||
  ...
  async getVoiceInsightSummary(
    ...
    const avgSentiment =
      recentSessions.reduce((sum, s) => sum + s.analysis.sentiment.score, 0) /
    ...
    const avgSpeechRate =
      recentSessions.reduce((sum, s) => sum + s.analysis.speechPatterns.speechRate, 0) /
      recentSessions.length;
  ...
  private analyzeVoiceTrends(sessions: VoiceSession[]): Array<{
    ...
    const firstHalfSentiment =
      firstHalf.reduce((sum, s) => sum + s.analysis.sentiment.score, 0) /
      Math.max(1, firstHalf.length);

    const secondHalfSentiment =
      secondHalf.reduce((sum, s) => sum + s.analysis.sentiment.score, 0) /
    ...
    const recentSpeechRate =
      secondHalf.reduce((sum, s) => sum + s.analysis.speechPatterns.speechRate, 0) /
      Math.max(1, secondHalf.length);

    const olderSpeechRate =
      firstHalf.reduce((sum, s) => sum + s.analysis.speechPatterns.speechRate, 0) /
      Math.max(1, firstHalf.length);
  ...
  private async generateInsightSummary(
    ...
    const emotionDescription =
      avgSentiment > 0.3 ? 'generally positive' : avgSentiment < -0.3 ? 'challenging' : 'balanced';

    const summary =
      `Over the past ${sessions.length} voice sessions, your emotional state has been ${emotionDescription}. ` +
      `Your dominant emotion has been ${dominantEmotion}, appearing in ${Math.round(
        (sessions.filter(
          s =>
            Object.entries(s.analysis.sentiment.emotions).sort((a, b) => b[1] - a[1])[0][0] ===
            dominantEmotion
        ).length /
          sessions.length) *
          100
  ...
  private generateVoiceRecommendations(
    ...
    if (avgSentiment < -0.2) {
      recommendations.push(
        'Consider incorporating mood-lifting activities into your daily routine'
      );
    ...
    const fillerWordAvg =
      sessions.reduce((sum, s) => sum + s.analysis.speechPatterns.fillerWords, 0) / sessions.length;

    if (fillerWordAvg > 5) {
      recommendations.push(
        'Practice speaking more confidently by preparing key points before recording'
      );
    }
  ...
  private calculateVocabularyGrowth(sessions: VoiceSession[]): number {
    ...
    const growth =
      (lastSession.analysis.linguisticAnalysis.vocabulary.sophistication -
  ...
  async compareVoiceSessions(
    ...
    const speechRateChange =
      session2.analysis.speechPatterns.speechRate - session1.analysis.speechPatterns.speechRate;
    const vocabularyChange =
      session2.analysis.linguisticAnalysis.vocabulary.sophistication -
  ...
  private generateComparisonInsights(
    ...
    const biggestEmotionChange = Object.entries(emotionChanges).sort(
      (a, b) => Math.abs(b[1]) - Math.abs(a[1])
    )[0];

    if (Math.abs(biggestEmotionChange[1]) > 0.3) {
      insights.push(
        `Notable change in ${biggestEmotionChange[0]}: ${
