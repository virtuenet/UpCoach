...
export class InsightGenerator {
  ...
  private generationStrategies!: Map<InsightType, (data: unknown) => Promise<Insight[]>>;
  ...
  private async gatherUserData(userId: string, startDate: Date, endDate: Date): Promise<unknown> {
  ...
  private async generatePatternInsights(data: unknown): Promise<Insight[]> {
  ...
  private async generateAnomalyInsights(data: unknown): Promise<Insight[]> {
    const insights: Insight[] = [];
    ...
    if (moodAnomaly) {
      insights.push({
        ...
        actionItems: moodAnomaly.actions.map((action: unknown, idx: number) => ({
  ...
  private async generatePredictionInsights(data: unknown): Promise<Insight[]> {
    const insights: Insight[] = [];

    // Goal completion predictions
    for (const goal of data.goals.filter((g: unknown) => g.status === 'active')) {
  ...
  private async generateRecommendationInsights(data: unknown): Promise<Insight[]> {
    const insights: Insight[] = [];
  ...
  private async generateAchievementInsights(data: unknown): Promise<Insight[]> {
    const insights: Insight[] = [];
        ...
        evidence: [
          {
            ...
            dataPoints: data.goals
              .filter((g: unknown) => g.status === 'completed')
              .map((g: unknown) => ({
                title: g.title,
  ...
  private async generateRiskInsights(data: unknown): Promise<Insight[]> {
    const insights: Insight[] = [];
  ...
  private async generateOpportunityInsights(data: unknown): Promise<Insight[]> {
    const insights: Insight[] = [];
  ...
  private async generateCorrelationInsights(data: unknown): Promise<Insight[]> {
    const insights: Insight[] = [];
  ...
  private analyzeTaskPatterns(tasks: Task[]): unknown {
    const patterns = {
      strongPatterns: [] as unknown[],
      heatmapData: {} as unknown,
    };
  ...
  private buildProductivityHeatmap(tasks: Task[]): unknown {
  ...
  private analyzeMoodPatterns(moods: Mood[]): unknown {
    if (moods.length < 5) {
  ...
  private detectMoodAnomaly(moods: Mood[]): unknown {
    ...
    const negativeCount = recentMoods.filter(
      (m: unknown) => m.mood === 'stressed' || m.mood === 'sad'
    ).length;
  ...
  private generateProgressProjection(goal: Goal, _prediction: unknown): unknown {
  ...
  private calculateTaskMoodCorrelation(tasks: Task[], moods: Mood[]): unknown {
    // Group by day
  ...
  private async analyzeTrends(userData: unknown): Promise<Trend[]> {
    const trends: Trend[] = [];
  ...
  private calculateTrend(items: unknown[], _dateField: string, filterFn?: (item: unknown) => boolean): unknown {
  ...
  private calculateMoodTrend(moods: Mood[]): unknown {
    const moodValues: Record<string, number> = {
      happy: 5,
      content: 4,
      neutral: 3,
  ...
  private calculateEngagementTrend(userData: unknown): unknown {
  ...
  private analyzeProductiveHours(tasks: Task[]): unknown {
  ...
  private calculateTaskVelocity(tasks: Task[]): number {
    const days = new Set(tasks.map((t: unknown) => new Date(t.createdAt).toDateString())).size;
