...
Provide practical strategies and emotional support.`,
  ...
  optimizeMessages(messages: unknown[], options: unknown): unknown[] {
  ...
  generatePersonalizedPrompt(basePrompt: string, userContext: unknown, promptType: string): string {
  ...
  private summarizeEarlyMessages(messages: unknown[]): string {
  ...
  private enhanceBasePrompt(basePrompt: string, userContext: unknown): string {
  ...
  private extractContextValue(context: unknown, variable: string): string {
  ...
  private addPersonalizationLayers(prompt: string, context: unknown): string {
    const layers: string[] = [];
  ...
  getBestPromptTemplate(category: string, userContext: unknown): PromptTemplate | null {
  ...
  private getContextualRelevance(template: PromptTemplate, context: unknown): number {
    let relevance = 1.0;
