...
export interface DashboardMetrics {
  ...
  performanceTrends: {
    hourly: Array<{ hour: string; metrics: any }>;
    daily: Array<{ date: string; metrics: any }>;
    weekly: Array<{ week: string; metrics: any }>;
  };
  alerts: Array<{
    id: string;
...
export class RealTimeDashboardService extends EventEmitter {
   ...
   */
  private async handleMetricsRequest(clientId: string, data: any): Promise<void> {
    const client = this.connectedClients.get(clientId);
    if (!client) return;
   ...
   */
  private async fetchCustomMetrics(filters: any, options: any): Promise<any> {
   ...
   */
  private async handlePlatformSync(data: any): Promise<void> {
   ...
   */
  private async handleWebhookUpdate(data: any): Promise<void> {
   ...
   */
  public async getWidgetData(widgetId: string, config: any): Promise<DashboardWidget> {
    let data;
   ...
   */
  private async fetchMetricWidget(config: any): Promise<any> {
   ...
   */
  private async fetchChartWidget(config: any): Promise<any> {
   ...
   */
  private async fetchTableWidget(config: any): Promise<any> {
   ...
   */
  private async fetchHeatmapWidget(config: any): Promise<any> {
   ...
   */
  private async fetchFunnelWidget(config: any): Promise<any> {
   ...
   */
  private async fetchTimeSeriesData(metrics: string[], dateRange: any): Promise<any> {
   ...
   */
  private async fetchBarChartData(metrics: string[], groupBy: string, dateRange: any): Promise<any> {
    const metricSelects = metrics.map(m => `SUM(${m}) as ${m}`).join(', ');
   ...
   */
  private async fetchPieChartData(metric: string, groupBy: string, dateRange: any): Promise<any> {
    const query = `
      SELECT
        ${groupBy} as label,
        SUM(${metric}) as value
