...
export class AdPlatformIntegrationService extends EventEmitter {
   ...
   */
  private async syncGoogleAds(): Promise<any> {
    if (!this.googleAdsClient) {
   ...
   * Sync Meta Ads data
   */
  private async syncMetaAds(): Promise<any> {
    if (!this.metaAdsClient) {
   ...
   */
  private async syncLinkedInAds(): Promise<any> {
    if (!this.linkedInAdsClient) {
  ...
  private setupWebhooks(): void {
    // Google Ads webhook
    this.webhookHandlers.set('google', async (data: any) => {
      try {
    ...
    // Meta Ads webhook
    this.webhookHandlers.set('meta', async (data: any) => {
      try {
    ...
    this.webhookHandlers.set('linkedin', async (data: any) => {
      try {
