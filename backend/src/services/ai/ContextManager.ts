...
export class ContextManager {
  ...
  private getTodayAccomplishments(tasks: any[]): string {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
  ...
  private async extractConversationPatterns(messages: any[]): Promise<any> {
    const patterns = {
  ...
  async enrichMessages(messages: any[], context: any): Promise<any[]> {
