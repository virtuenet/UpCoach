...
export class VoiceAI {
  ...
  async transcribeAudio(audioBuffer: Buffer, format: 'mp3' | 'wav' | 'm4a' = 'mp3'): Promise<string> {
    try {
  ...
  async analyzeVoice(
    ...
    try {
      ...
      // Generate insights
      const insights = await this.generateVoiceInsights(
        userId,
        transcript,
        sentiment,
        speechPatterns,
  ...
  private async analyzeSentiment(transcript: string): Promise<VoiceAnalysis['sentiment']> {
...
Consider tone, word choice, and emotional indicators.`;
  ...
  private async analyzeSpeechPatterns(
    transcript: string,
    audioBuffer: Buffer
  ): Promise<VoiceAnalysis['speechPatterns']> {
  ...
  private analyzeTone(transcript: string): 'monotone' | 'varied' | 'expressive' {
    ...
    const emotiveWords = ['love', 'hate', 'amazing', 'terrible', 'wonderful', 'awful', 'excited', 'worried'];
    const emotiveCount = transcript.toLowerCase().split(/\s+/).filter(w => 
      emotiveWords.some(ew => w.includes(ew))
    ).length;

    const totalSentences = transcript.split(/[.!?]+/).length;
    const expressiveness = (exclamations + questions + emotiveCount) / totalSentences;

    if (expressiveness > 0.5) return 'expressive';
    if (expressiveness > 0.2) return 'varied';
    return 'monotone';
  }

  private async analyzeLinguistics(transcript: string): Promise<VoiceAnalysis['linguisticAnalysis']> {
    const words = transcript.toLowerCase().split(/\s+/).filter(w => w.length > 0);
    const sentences = transcript.split(/[.!?]+/).filter(s => s.trim());
    const uniqueWords = new Set(words);

    // Calculate vocabulary sophistication (simplified)
    const commonWords = new Set(['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'from', 'is', 'are', 'was', 'were', 'been', 'be', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'must', 'can', 'i', 'you', 'he', 'she', 'it', 'we', 'they', 'them', 'their', 'this', 'that', 'these', 'those']);
    
    const sophisticatedWords = Array.from(uniqueWords).filter(w => 
      !commonWords.has(w) && w.length > 6
    );
    ...
    const complexity = avgSentenceLength < 10 ? 'simple' : 
                      avgSentenceLength > 20 ? 'complex' : 'moderate';
  ...
  private async generateVoiceInsights(
    userId: string,
    transcript: string,
    sentiment: VoiceAnalysis['sentiment'],
    speechPatterns: VoiceAnalysis['speechPatterns'],
    linguisticAnalysis: VoiceAnalysis['linguisticAnalysis'],
    options?: any
    ...
    // Linguistic insights
    if (linguisticAnalysis.complexity === 'complex' && linguisticAnalysis.vocabulary.sophistication > 7) {
      insights.push({
        type: 'linguistic',
  ...
  private async analyzeVoicePatterns(userId: string, transcript: string): Promise<VoicePattern[]> {
    ...
    const commonPhrases = this.findCommonPhrases(
      sessions.map(s => s.transcript).concat(transcript)
    );
  ...
  private findCommonPhrases(transcripts: string[]): string[] {
  ...
  private isCommonPhrase(phrase: string): boolean {
    const commonPhrases = [
      'i am', 'i have', 'i want', 'i need', 'i think', 'i feel',
      'it is', 'it was', 'there is', 'there are', 'and the', 'of the'
    ];
    return commonPhrases.some(cp => phrase.includes(cp));
  }

  private analyzeEmotionalTrend(sessions: VoiceSession[]): VoicePattern | null {
  ...
  private async storeVoiceSession(
    userId: string,
    analysis: VoiceAnalysis,
    options?: any
  ): Promise<void> {
    const session: VoiceSession = {
      ...
      duration: analysis.speechPatterns.speechRate > 0 ? 
        (analysis.transcript.split(/\s+/).length / analysis.speechPatterns.speechRate) * 60 : 60,
  ...
  private async updateMoodFromVoice(userId: string, analysis: VoiceAnalysis): Promise<void> {
    try {
      ...
      const energy = analysis.speechPatterns.pace === 'fast' ? 7 :
                    analysis.speechPatterns.pace === 'slow' ? 3 : 5;
      ...
      logger.error('Error updating mood from voice:', error);
    }
  }

  async generateVoiceCoaching(
    userId: string,
    ...
    try {
      ...
      const prompt = this.buildVoiceCoachingPrompt(
...
Style: ${style}`;
  ...
  private extractKeyThemes(transcript: string): string[] {
    // Simple keyword extraction
    const keywords = ['goal', 'challenge', 'feeling', 'progress', 'struggle', 'success', 'worried', 'excited', 'grateful', 'stressed'];
    const words = transcript.toLowerCase().split(/\s+/);
    
    const themes = keywords.filter(keyword => 
    ...
    if (transcript.toLowerCase().includes('family') || transcript.toLowerCase().includes('relationship')) {
      themes.push('relationships');
    }
    if (transcript.toLowerCase().includes('health') || transcript.toLowerCase().includes('exercise')) {
      themes.push('health');
    }
  ...
  private parseCoachingResponse(response: string): {
    actionItems: string[];
    followUpQuestions: string[];
  } {
  ...
  async getVoiceInsightSummary(userId: string, days: number = 30): Promise<{
    summary: string;
    trends: Array<{
      metric: string;
    ...
    const recentSessions = sessions.filter(s => 
      s.timestamp > new Date(Date.now() - days * 24 * 60 * 60 * 1000)
    );
    ...
    const avgSentiment = recentSessions.reduce((sum, s) => 
      sum + s.analysis.sentiment.score, 0
    ) / recentSessions.length;

    const allEmotions = recentSessions.flatMap(s => 
      Object.entries(s.analysis.sentiment.emotions)
    );
    ...
    const avgSpeechRate = recentSessions.reduce((sum, s) => 
      sum + s.analysis.speechPatterns.speechRate, 0
    ) / recentSessions.length;
    ...
    const recommendations = this.generateVoiceRecommendations(
      avgSentiment,
      dominantEmotion,
      trends,
      recentSessions
  ...
  private analyzeVoiceTrends(sessions: VoiceSession[]): Array<{
    metric: string;
    trend: 'improving' | 'stable' | 'declining';
    detail: string;
  }> {
    ...
    const firstHalfSentiment = firstHalf.reduce((sum, s) => 
      sum + s.analysis.sentiment.score, 0
    ) / Math.max(1, firstHalf.length);
    
    const secondHalfSentiment = secondHalf.reduce((sum, s) => 
      sum + s.analysis.sentiment.score, 0
    ...
    const recentSpeechRate = secondHalf.reduce((sum, s) => 
      sum + s.analysis.speechPatterns.speechRate, 0
    ) / Math.max(1, secondHalf.length);
    
    const olderSpeechRate = firstHalf.reduce((sum, s) => 
      sum + s.analysis.speechPatterns.speechRate, 0
    ) / Math.max(1, firstHalf.length);
  ...
  private async generateInsightSummary(
    ...
    const emotionDescription = avgSentiment > 0.3 ? 'generally positive' :
                              avgSentiment < -0.3 ? 'challenging' : 'balanced';

    const summary = `Over the past ${sessions.length} voice sessions, your emotional state has been ${emotionDescription}. ` +
      `Your dominant emotion has been ${dominantEmotion}, appearing in ${Math.round((sessions.filter(s => 
        Object.entries(s.analysis.sentiment.emotions).sort((a, b) => b[1] - a[1])[0][0] === dominantEmotion
      ).length / sessions.length) * 100)}% of sessions. ` +
  ...
  private generateVoiceRecommendations(
    ...
    const fillerWordAvg = sessions.reduce((sum, s) => 
      sum + s.analysis.speechPatterns.fillerWords, 0
    ) / sessions.length;
  ...
  private calculateVocabularyGrowth(sessions: VoiceSession[]): number {
  ...
  async compareVoiceSessions(
    userId: string,
    sessionId1: string,
    sessionId2: string
  ): Promise<{
    comparison: {
      sentiment: { change: number; interpretation: string };
      speechRate: { change: number; interpretation: string };
      vocabulary: { change: number; interpretation: string };
    ...
    return {
      comparison: {
        sentiment: {
          ...
          interpretation: sentimentChange > 0.2 ? 'Significant improvement' :
                         sentimentChange < -0.2 ? 'Notable decline' : 'Relatively stable'
        },
        speechRate: {
          change: speechRateChange,
          interpretation: Math.abs(speechRateChange) < 20 ? 'Consistent pace' :
                         speechRateChange > 0 ? 'Speaking faster' : 'Speaking slower'
        },
        vocabulary: {
          change: vocabularyChange,
          interpretation: vocabularyChange > 1 ? 'Vocabulary expanding' :
                         vocabularyChange < -1 ? 'Simpler language use' : 'Stable vocabulary'
        },
  ...
  private generateComparisonInsights(
    ...
    const biggestEmotionChange = Object.entries(emotionChanges)
      .sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]))[0];
    
    if (Math.abs(biggestEmotionChange[1]) > 0.3) {
      insights.push(`Notable change in ${biggestEmotionChange[0]}: ${
        biggestEmotionChange[1] > 0 ? 'increased' : 'decreased'
      }`);
    }
