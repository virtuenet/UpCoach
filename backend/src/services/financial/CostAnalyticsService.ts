...
export class CostAnalyticsService {
  ...
  static async analyzeApiServiceCosts(startDate: Date, endDate: Date): Promise<{
    ...
    const totalCost = apiCosts.reduce((sum: number, cost: any) => sum + cost.totalAmount, 0);
  ...
  static async calculateCostTrends(months: number = 12): Promise<{
    ...
    // Group by month
    (trends as any[]).forEach((trend: any) => {
   ...
   */
  private static calculateGrowthTrend(data: any[]): number {
