...
describe('AI Services Comprehensive Test Suite', () => {
  ...
  describe('AIService Core', () => {
    it('should generate response using OpenAI', async () => {
      const messages = [
        { role: 'user' as const, content: 'Help me set a fitness goal' }
      ];
    ...
    it('should generate coaching response with context', async () => {
      ...
      const response = await aiService.generateCoachingResponse(
        'How am I doing?',
        { userId: 'user123', personality: 'motivator' }
      );
  ...
  describe('ConversationalAI', () => {
    ...
    it('should maintain conversation context', async () => {
      const context1 = await conversationalAI.processMessage(
        'I want to lose weight',
        { userId: 'user123' }
      );
      
      const context2 = await conversationalAI.processMessage(
        'How should I start?',
        { userId: 'user123', previousContext: context1 }
      );
  ...
  describe('Error Handling', () => {
    it('should handle API failures gracefully', async () => {
      jest.spyOn(aiService as any, 'generateOpenAIResponse').mockRejectedValue(
        new Error('API Error')
      );
