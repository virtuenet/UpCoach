...
const AdminCoachMarketplace: React.FC = () => {
  ...
  const handleFeatureCoach = async (coachId: number, featured: boolean) => {
    try {
      ...
      enqueueSnackbar(featured ? 'Coach featured successfully' : 'Coach unfeatured successfully', {
        variant: 'success',
      });
      fetchData();
    } catch (error) {
  ...
  const filteredCoaches = coaches.filter(coach => {
    ...
    const matchesStatus =
      statusFilter === 'all' || (statusFilter === 'active' ? coach.isActive : !coach.isActive);
    const matchesVerification =
      verificationFilter === 'all' ||
      (verificationFilter === 'verified' ? coach.isVerified : !coach.isVerified);
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                ...
                <Box>
                  ...
                  <Typography variant="h5">{stats?.totalCoaches || 0}</Typography>
                </Box>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', mt: 2 }}>
                <VerifiedUser sx={{ fontSize: 20, color: 'success.main', mr: 1 }} />
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card>
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                <EventAvailable sx={{ fontSize: 40, color: 'info.main', mr: 2 }} />
                <Box>
                  <Typography color="textSecondary" variant="body2">
                    Total Sessions
                  </Typography>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card>
            <CardContent>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mt: 2 }}>
                <TrendingUp sx={{ fontSize: 20, color: 'success.main', mr: 1 }} />
                <Typography variant="body2" color="success.main">
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                ...
                <Box>
                  ...
                  <Typography variant="h5">{stats?.averageRating?.toFixed(1) || '0.0'}</Typography>
                </Box>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', mt: 2 }}>
            ...
            <TableBody>
              {filteredCoaches.map(coach => (
                <TableRow key={coach.id}>
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Rating value={coach.averageRating} readOnly size="small" />
                  ...
                  <TableCell>${coach.hourlyRate?.toFixed(2) || 'N/A'}</TableCell>
                  <TableCell>
                    <Chip
                  ...
                  <TableCell>
                    ...
                    <Tooltip title={coach.isActive ? 'Deactivate' : 'Activate'}>
                      <IconButton
                        size="small"
                        onClick={() => handleToggleCoachStatus(coach.id, coach.isActive)}
                      >
                    ...
                    {!coach.isVerified && (
                      <Tooltip title="Verify Coach">
                        <IconButton size="small" onClick={() => handleVerifyCoach(coach.id)}>
                          <VerifiedUser />
                        </IconButton>
                      </Tooltip>
                    )}
                    <Tooltip title={coach.isFeatured ? 'Unfeature' : 'Feature'}>
                      <IconButton
                        size="small"
                        onClick={() => handleFeatureCoach(coach.id, !coach.isFeatured)}
                      >
                        <Star color={coach.isFeatured ? 'warning' : 'inherit'} />
                      </IconButton>
                    </Tooltip>
                  </TableCell>
                </TableRow>
            ...
            <TableBody>
              {sessions.map(session => (
                <TableRow key={session.id}>
                  ...
                  <TableCell>
                    {format(parseISO(session.scheduledAt), 'MMM dd, yyyy HH:mm')}
                  </TableCell>
            ...
            <TableBody>
              {reviews.map(review => (
                <TableRow key={review.id}>
                  ...
                  <TableCell>{format(parseISO(review.createdAt), 'MMM dd, yyyy')}</TableCell>
                  <TableCell>
                    <IconButton
                      size="small"
                      color="error"
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {(stats?.sessionTypes || []).map((entry: any, index: number) => (
          ...
          <Grid item xs={12} md={6}>
            <Card>
              <CardContent>
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={stats?.topSpecializations || []} layout="horizontal">
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis type="number" />
          ...
          {selectedCoach && (
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={8}>
                <Typography variant="h6">{selectedCoach.displayName}</Typography>
                ...
                <Box sx={{ mb: 2 }}>
                  ...
                  <Typography variant="body2">• Timezone: {selectedCoach.timezone}</Typography>
                  <Typography variant="body2">
                    • Hourly Rate: ${selectedCoach.hourlyRate?.toFixed(2) || 'N/A'}
                  </Typography>
                </Box>
                <Box>
                  <Typography variant="subtitle2" gutterBottom>
                  ...
                  {selectedCoach.certifications?.map((cert: any, index: number) => (
