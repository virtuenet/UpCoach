import React, { useState, useEffect, useCallback, useMemo, useRef, useContext, ReactNode } from 'react';
    ...
    <div className="space-y-6">
          ...
          {Object.entries(moodStats.distribution).map(([mood, count]) => (
            <div key={mood} className="bg-white p-4 rounded-lg shadow">
              <div className="flex items-center justify-between">
                ...
                <div className="text-2xl">{moodEmojis[mood as keyof typeof moodEmojis]}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-medium text-gray-900">Recent Mood Entries</h3>
        </div>
                ...
                <tr key={entry.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <span className="text-xl mr-2">{moodEmojis[entry.moodLevel]}</span>
                      <span
