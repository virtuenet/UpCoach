...
const SEOHelper: React.FC<SEOHelperProps> = ({ title, description, content, onUpdate }) => {
  ...
  const analyzeSEO = () => {
    ...
    // Title analysis
    if (!seoTitle) {
      issues.push({ type: 'error', message: 'SEO title is missing', field: 'title' });
      score -= 20;
    } else {
      if (seoTitle.length < 30) {
        issues.push({ type: 'warning', message: 'SEO title is too short (min 30 chars)', field: 'title' });
        score -= 10;
      } else if (seoTitle.length > 60) {
        issues.push({ type: 'warning', message: 'SEO title is too long (max 60 chars)', field: 'title' });
        score -= 5;
      } else {
        issues.push({ type: 'success', message: 'SEO title length is optimal', field: 'title' });
      }
    }

    // Description analysis
    if (!seoDescription) {
      issues.push({ type: 'error', message: 'SEO description is missing', field: 'description' });
      score -= 20;
    } else {
      if (seoDescription.length < 120) {
        issues.push({ type: 'warning', message: 'SEO description is too short (min 120 chars)', field: 'description' });
        score -= 10;
      } else if (seoDescription.length > 160) {
        issues.push({ type: 'warning', message: 'SEO description is too long (max 160 chars)', field: 'description' });
        score -= 5;
      } else {
        issues.push({ type: 'success', message: 'SEO description length is optimal', field: 'description' });
      }
    }
    ...
    if (wordCount < 300) {
      issues.push({ type: 'warning', message: 'Content is too short for good SEO (min 300 words)' });
      score -= 15;
    } else if (wordCount > 300 && wordCount < 1000) {
      issues.push({ type: 'success', message: 'Content length is good' });
    } else if (wordCount >= 1000) {
      issues.push({ type: 'success', message: 'Content length is excellent for SEO' });
    }
    ...
    if (keywords.length === 0) {
      ...
      keywords.forEach(keyword => {
        ...
        if (density === 0) {
          issues.push({ type: 'warning', message: `Keyword "${keyword}" not found in content` });
          score -= 5;
        } else if (density > 3) {
          issues.push({ type: 'warning', message: `Keyword "${keyword}" might be overused (${density.toFixed(1)}%)` });
          score -= 3;
        }
      });
    }
    ...
    if (h1Count === 0) {
      issues.push({ type: 'warning', message: 'No H1 heading found in content' });
      score -= 10;
    }
    
    if (h2Count === 0) {
      issues.push({ type: 'warning', message: 'Consider adding H2 headings for better structure' });
      score -= 5;
    }
    ...
    if (imgCount > 0 && altMissing > 0) {
      issues.push({ type: 'warning', message: `${altMissing} image(s) missing alt text` });
      score -= 5;
    }
...
const commonWords = [
  ...
  'getting', 'made', 'being', 'where', 'much', 'too', 'very', 'still', 'being', 'going'
];
