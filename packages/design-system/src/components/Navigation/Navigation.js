...
export const Navigation = ({ items, logo, title = 'UpCoach', user, notifications = 0, onNavigate, onLogout, variant = 'permanent', mini = false, position = 'left', currentPath = '', }) => {
    ...
    const drawerContent = (_jsxs(_Fragment, { children: [_jsxs(DrawerHeader, { children: [_jsxs(Box, { sx: { display: 'flex', alignItems: 'center', gap: 1 }, children: [logo, !mini && (_jsx(Typography, { variant: "h6", noWrap: true, children: title }))] }), variant !== 'temporary' && (_jsx(IconButton, { onClick: handleDrawerToggle, children: theme.direction === 'ltr' ? _jsx(ChevronLeftIcon, {}) : _jsx(ChevronRightIcon, {}) }))] }), _jsx(Divider, {}), _jsx(List, { children: renderNavItems(items) })] }));
    const appBarContent = (_jsx(AppBar, { position: "fixed", children: _jsxs(Toolbar, { children: [_jsx(IconButton, { color: "inherit", "aria-label": "open drawer", onClick: handleDrawerToggle, edge: "start", sx: { mr: 2, ...(open && position === 'left' && { display: 'none' }) }, children: _jsx(MenuIcon, {}) }), _jsx(Typography, { variant: "h6", noWrap: true, component: "div", sx: { flexGrow: 1 }, children: title }), notifications > 0 && (_jsx(IconButton, { color: "inherit", children: _jsx(Badge, { badgeContent: notifications, color: "error", children: _jsx(NotificationsIcon, {}) }) })), user && (_jsxs(_Fragment, { children: [_jsx(Tooltip, { title: "Account", children: _jsx(IconButton, { onClick: handleUserMenuOpen, color: "inherit", children: user.avatar ? (_jsx(Avatar, { src: user.avatar, alt: user.name, sx: { width: 32, height: 32 } })) : (_jsx(AccountIcon, {})) }) }), _jsxs(Menu, { anchorEl: anchorEl, open: Boolean(anchorEl), onClose: handleUserMenuClose, children: [_jsxs(Box, { sx: { px: 2, py: 1 }, children: [_jsx(Typography, { variant: "subtitle1", children: user.name }), user.email && (_jsx(Typography, { variant: "body2", color: "text.secondary", children: user.email }))] }), _jsx(Divider, {}), _jsx(MenuItem, { onClick: () => {
                                        handleUserMenuClose();
                                        onNavigate?.('/settings');
                                    }, children: "Settings" }), _jsx(MenuItem, { onClick: () => {
                                        handleUserMenuClose();
                                        onLogout?.();
                                    }, children: "Logout" })] })] }))] }) }));
    if (position === 'top') {
    ...
    return (_jsxs(_Fragment, { children: [appBarContent, _jsx(StyledDrawer, { variant: isMobile ? 'temporary' : variant, mini: mini, open: open, onClose: handleDrawerToggle, ModalProps: { keepMounted: true }, children: drawerContent })] }));
};
