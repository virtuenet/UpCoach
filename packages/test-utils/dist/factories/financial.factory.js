...
export const FinancialSnapshotFactory = Factory.define(({ params }) => {
    const revenue = params.revenue || faker.number.float({ min: 10000, max: 100000, fractionDigits: 2 });
    const costs = params.costs || revenue * faker.number.float({ min: 0.3, max: 0.7, fractionDigits: 2 });
    return {
...
export const TransactionFactory = Factory.define(({ params }) => ({
    id: params.id || faker.string.uuid(),
...
export const SubscriptionFactory = Factory.define(({ params }) => {
...
// Helper functions
export function createMonthlySnapshots(months = 12) {
    const snapshots = [];
    const now = new Date();
    for (let i = months - 1; i >= 0; i--) {
        const date = new Date(now.getFullYear(), now.getMonth() - i, 1);
        const mrr = 10000 + (months - i) * 1000; // Growing MRR
        snapshots.push(FinancialSnapshotFactory.build({
            date,
            mrr,
            arr: mrr * 12,
...
export function createSubscriptionHistory(userId, months = 6) {
    const subscriptions = [];
    const now = new Date();
    for (let i = months - 1; i >= 0; i--) {
        const startDate = new Date(now.getFullYear(), now.getMonth() - i, 1);
        const endDate = new Date(now.getFullYear(), now.getMonth() - i + 1, 1);
        subscriptions.push(SubscriptionFactory.build({
            userId,
...
//# sourceMappingURL=financial.factory.js.map