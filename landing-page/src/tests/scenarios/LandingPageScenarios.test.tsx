...
describe('Landing Page User Scenarios', () => {
  ...
  describe('Scenario 2: User interested in pricing', () => {
    test('user navigates directly to pricing and selects a plan', async () => {
      ...
      expect(proButton.closest('a')).toHaveAttribute('href', 'https://apps.apple.com/app/upcoach');
    });
    ...
    test('user compares plans and reads FAQs before deciding', async () => {
      ...
      expect(screen.getByText('5 voice journal entries/day')).toBeInTheDocument();
      expect(screen.getByText('Unlimited voice journal entries')).toBeInTheDocument();
  ...
  describe('Scenario 3: Lead generation flow', () => {
    test('user fills lead capture form after 45 seconds', async () => {
      ...
      // Verify submission
      await waitFor(() => {
        expect(trackFormSubmit).toHaveBeenCalledWith('lead_capture', 'modal-time-based');
      });
    ...
    test('user signs up through inline lead generation section', async () => {
      ...
      await user.type(emailFields[emailFields.length - 1], 'john@enterprise.com');
      
      // Submit
      const submitButtons = screen.getAllByRole('button', { name: /Get Early Access/i });
      await user.click(submitButtons[submitButtons.length - 1]);
      
      // Verify tracking
      await waitFor(() => {
        expect(trackFormSubmit).toHaveBeenCalledWith('lead_capture', 'lead-generation-section');
      });
    });
  });
  ...
  describe('Scenario 5: Trust and social proof evaluation', () => {
    test('user reviews testimonials and success stories', () => {
      ...
      expect(screen.getByText(/Real Stories, Real Results/i)).toBeInTheDocument();
  ...
  describe('Scenario 6: Return visitor conversion', () => {
    test('returning user skips modal and goes directly to download', async () => {
      ...
      const appDownloadSection = screen.getByText(/Start Your Transformation Journey Today/i);
      ...
      // Should navigate to app store
      expect(iosButton).toHaveAttribute('href', 'https://apps.apple.com/app/upcoach');
    });
  });
