...
test.describe("Lead Capture", () => {
  test("lead capture modal appears after 30 seconds", async ({ page }) => {
    ...
    await expect(
      page.getByText(/get your free productivity guide/i),
    ).not.toBeVisible();
  ...
  test("exit intent triggers modal on desktop", async ({
    page,
    browserName,
  }) => {
...
test.describe("Contact Form", () => {
  ...
  test("contact form submits successfully", async ({ page }) => {
    ...
    await expect(
      page.getByText(/thank you! we'll get back to you within 24 hours/i),
    ).toBeVisible();
  });
