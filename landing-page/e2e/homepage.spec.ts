...
test.describe('Homepage', () => {
  ...
  test('hero section renders correctly', async ({ page }) => {
    ...
    // Check CTA buttons
    await expect(page.getByRole('link', { name: /download for ios/i })).toBeVisible();
  ...
  test('app store badges have correct links', async ({ page }) => {
    ...
    const androidLink = page.getByRole('link', { name: /download.*android/i }).first();
    
    await expect(iosLink).toHaveAttribute('href', 'https://apps.apple.com/app/upcoach');
    await expect(androidLink).toHaveAttribute('href', 'https://play.google.com/store/apps/details?id=com.upcoach.app');
