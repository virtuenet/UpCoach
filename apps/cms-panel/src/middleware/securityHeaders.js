 ...
 */
export function configureSecurityHeaders(config = {}) {
    return (req, res, next) => {
        try {
            ...
            if (config.csp?.enabled !== false) {
                ...
                // Generate CSP header
                const cspHeader = cspService.generateCSPHeader(nonce, cspService.getEnvironmentDirectives(), {
 ...
 */
export function viteSecurityHeaders(config = {}) {
    return {
 ...
 */
export function handleCSPViolation(req, res) {
    try {
