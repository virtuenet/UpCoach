 ...
 */
const _STRONG_PASSWORD_REGEX = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/;
/**
 ...
 */
export function validateText(value, rules = {}) {
    const errors = [];
    let sanitized = value;
    // Trim whitespace
...
export function validatePassword(password, options = {}) {
    const { minLength = 12, requireUppercase = true, requireLowercase = true, requireNumbers = true, requireSpecialChars = true, checkStrength = true, } = options;
    const errors = [];
    if (!password) {
    ...
    if (checkStrength) {
        ...
        if (!/(.)\1{2,}/.test(password))
            strength++; // No repeated characters
        if (strength < 5) {
...
export function sanitizeHTML(html, options = {}) {
    const { allowedTags = [
        ...
        'li',
        'blockquote',
    ], allowedAttributes = ['class', 'id'], allowImages = false, allowLinks = false, } = options;
...
export function validateFile(file, options = {}) {
    ...
    allowedTypes = [], allowedExtensions = [], } = options;
    const errors = [];
    // Check file size
