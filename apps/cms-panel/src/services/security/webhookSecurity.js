...
class WebhookSecurityService {
    constructor() {
        ...
        this.DEFAULT_MAX_SIZE = 5 * 1024 * 1024; // 5MB
        this.DEFAULT_REPLAY_WINDOW = 300; // 5 minutes
     ...
     */
    async validateWebhook(name, payload) {
     ...
     */
    generateWebhookSignature(name, payload, timestamp) {
        const config = this.configs.get(name);
        if (!config) {
     ...
     */
    generateWebhookHeaders(name, payload) {
     ...
     */
    generateSignature(config, payload, timestamp) {
     ...
     */
    generateRequestId(name, payload, timestamp) {
     ...
     */
    async testWebhook(name, testUrl) {
        const config = this.configs.get(name);
        if (!config) {
            return {
                success: false,
...
export function webhookSecurityMiddleware(webhookName) {
