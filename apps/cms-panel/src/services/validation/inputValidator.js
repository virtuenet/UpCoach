...
class InputValidationService {
    ...
    sanitizeHTML(html, options) {
        return DOMPurify.sanitize(html, {
            ...
            ALLOWED_URI_REGEXP: new RegExp(`^(?:(?:${(options?.allowedSchemes || ['https']).join('|')}):)`, 'i'),
        });
    }
    /**
     ...
     */
    validateOrderBy(field, allowedFields) {
    ...
    detectInjection(input) {
        const patterns = [
            {
                name: 'SQL Injection',
                regex: /(\b(SELECT|INSERT|UPDATE|DELETE|DROP|UNION|ALTER|CREATE)\b)|(--)|(;)|(\|\|)|(\/\*)|(\*\/)/i,
            },
            {
                name: 'NoSQL Injection',
                regex: /(\$where)|(\$ne)|(\$gt)|(\$lt)|(\$regex)|(\$exists)|(\$type)|(\$mod)|(\$text)|(\$where)/i,
            },
            {
...
export const validationMiddleware = {
    pagination: inputValidator.middleware(z.object({
        page: commonSchemas.page,
        limit: commonSchemas.limit,
        sortBy: commonSchemas.sortBy.optional(),
        sortOrder: commonSchemas.sortOrder.optional(),
    })),
    id: inputValidator.middleware(z.object({
        id: commonSchemas.uuid,
    })),
    search: inputValidator.middleware(z.object({
