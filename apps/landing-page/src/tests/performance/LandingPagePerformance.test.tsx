...
describe('Landing Page Performance Tests', () => {
  ...
  describe('Component Render Performance', () => {
    ...
    test('Lead capture form renders quickly', async () => {
      const renderTime = await measureRenderTime(() => <LeadCaptureForm source="test" />);
  ...
  describe('Lazy Loading Performance', () => {
    test('Non-critical sections use lazy loading', () => {
      const { container } = render(
        <div>
          <div className="h-96 bg-gray-50 animate-pulse">Loading placeholder</div>
        </div>
      );
    ...
    test('Intersection Observer is used for lazy loading', () => {
      const observerCalls = (global.IntersectionObserver as jest.Mock).mock.calls;
  ...
  describe('Animation Performance', () => {
    test('Animations use CSS transforms for better performance', () => {
      ...
      const animatedElements = container.querySelectorAll('[class*="transform"]');
    ...
    test('Images use appropriate formats', () => {
      ...
      images.forEach(img => {
        ...
        if (src.includes('.')) {
          ...
          expect(['webp', 'avif', 'jpg', 'jpeg', 'png', 'svg']).toContain(format);
        }
      });
    });
  });
  ...
  describe('Cumulative Layout Shift (CLS)', () => {
    test('Components have defined dimensions to prevent layout shift', () => {
      ...
      const images = container.querySelectorAll('img');
      images.forEach(img => {
        expect(img.width > 0 || img.style.width || img.getAttribute('width')).toBeTruthy();
      });
      ...
      sections.forEach(section => {
        ...
        expect(styles.minHeight !== 'auto' || section.className.includes('min-h-')).toBeTruthy();
      });
    });
  });
