...
export function usePaginatedQuery<T = any>(
  ...
  page: number = 1,
  options?: Partial<UseQueryOptions<{ data: T[]; total: number; page: number; pageSize: number }, Error, { data: T[]; total: number; page: number; pageSize: number }, readonly unknown[]>>
) {
  return useApiQuery(
    [...queryKey, { page }],
    () => queryFn(page),
...
export function useInfiniteApiQuery<T = any>(
  ...
  options?: Partial<UseInfiniteQueryOptions<{
    ...
    nextPage?: number;
    hasMore: boolean;
  }, Error, { data: T[]; nextPage?: number; hasMore: boolean; }, any, any>>
) {
...
export function useUsers(filters?: any) {
  return useApiQuery(
    queryKeys.userList(filters),
    () => validatedApi.paginated('/users', filters)
  );
}
...
export function useContentItem(id: string) {
  return useApiQuery(
    queryKeys.contentItem(id),
    () => validatedApi.get(`/content/${id}`),
    {
      enabled: !!id,
    }
  );
